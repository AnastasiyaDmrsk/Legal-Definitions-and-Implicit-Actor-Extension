...
def _get_objects_from_head(token: Token) -> List[Token]:
    h = token.head
...
class SynsetFilter(CandidateFilter):
    ...
    def filter(self, target: ImplicitSubjectDetection, candidates: List[Token], _: FilterContext) -> List[Token]:
        ...
        filtered = list(filter(lambda x: self._is_valid(x) or any(self._is_valid(y) for y in _get_objects_from_head(x)),
                               candidates))
