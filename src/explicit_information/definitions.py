...
def find_definitions(soup):
    ...
    for element in start_class.next_elements:
...
def check_definition_part_of_another_definition(definition):
    ...
    for (key, value) in definitions_list:
        if definition in key and key != definition:
            part_def.add(key)
    return part_def
...
def check_two_definitions_in_text(definition1, definition2, text):
    if definition2 not in text:
        return False
    strings = text.split(definition2)
    new_string = "".join(strings)
    if definition1 in new_string:
        return False
    return True
...
def any_definition_in_text(text):
    ...
    for key, value in definitions_dict.items():
        ...
        if key in text or capitalized:
            ...
            if len(d) != 0:
                for k in d:
                    cap = k.capitalize() in text and k[0].islower()
                    if k in text or cap:
                        if cap:
                            k = k[0].upper() + k[1:]
